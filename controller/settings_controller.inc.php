...
class SettingsController extends GenericBlogCMSController
{
    ...
    */
    public function route($params)
    {
        ...
        switch($page_name)
        {
            ...
            $this->view->addScript(CLIENT_ROOT.'/resources/colorpicker/jscolor');
            $this->view->render(SERVER_PATH_BLOGCMS.'/app/view/settings/blogdesigner.php', array('blog' => $blog));
            break;
    ...
    **/
    ...
    {
        $settings_view = SERVER_PATH_BLOGCMS.'/app/view/settings/widgets_'.$widgetname.'.php';
	...
	**/
	public function action_updateBlogGeneral($blog)
    {
        ...
        // Redirect
		redirect(CLIENT_ROOT_BLOGCMS.'/config/'.$blog['id']);
	}
	...
	**/
    ...
    {
		return jsonToArray(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blogid.'/config.json');
	}
	...
	**/
	private function saveBlogConfig($blogid, $arrBlogConfig)
    {
		...
		file_put_contents(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blogid.'/config.json', $json_string);
	}
	...
	**/
    public function action_updateFooterContent($blog)
    {
		...
		// Output
		setSystemMessage(ITEM_UPDATED, "Success");
		redirect(CLIENT_ROOT_BLOGCMS.'/config/'.$blog['id'].'/footer');
    }
	...
	**/
    public function action_updateHeaderContent($blog)
    {
		...
		// Output
		setSystemMessage(ITEM_UPDATED, "Success");
		redirect(CLIENT_ROOT_BLOGCMS.'/config/'.$blog['id'].'/header');
    }
    ...
    public function action_addPage($blog)
    {
        ...
        if(strtolower(getType($targetpost)) != 'array' || $targetpost['blog_id'] != $blog['id'])
        {
            die("Unexpected Post ID!");
        }
	...
	**/
	...
	{
		$arraySettings = jsonToArray(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog['id'].'/template_config.json');
		// Loop through JSON array
		foreach($arraySettings as $group => $groupdata)
		{
			// echo "<b>processing {$group}</b><br>";
		...
		file_put_contents(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog['id'].'/template_config.json', json_encode($arraySettings));
    ...
    **/
    ...
    {
        // Update default.css
        $copy_css = SERVER_PATH_BLOGCMS.'/data/templates/stylesheets/'.$template_id.'.css';
        ...
        $copy_json = SERVER_PATH_BLOGCMS.'/data/templates/stylesheets/'.$template_id.'.json';
        $new_json = SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog_key.'/template_config.json';
    ...
    **/
    public function action_applyNewTemplate($DATA, $params)
    {
        ...
        // Redirect Home
        setSystemMessage(ITEM_UPDATED, "Success");
        redirect(CLIENT_ROOT_BLOGCMS.'/config/'.$blog_key);
    }
    ...
    **/
    ...
    {
		// Sanitize Variables
        $css_string = sanitize_string($_POST['fld_css']);
        $blog_id = safeNumber($params[0]);
		// Check we have permission to perform action
		if(!$this->modelContributors->isBlogContributor($blog_id, $_SESSION['userid'])) return $this->throwAccessDenied();
		// Update default.css
        if(is_dir(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog_id))
        {
            file_put_contents(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog_id.'/default.css', $css_string);
            setSystemMessage(ITEM_UPDATED, "Success");
        }
        else
        {
        ...
        redirect(CLIENT_ROOT_BLOGCMS.'/config/'.$blog_id.'/stylesheet');
    }
    ...
    **/
    ...
    {
		$arrayBlogConfig = jsonToArray(SERVER_PATH_BLOGCMS.'/data/blogs/'.$blog['id'].'/template_config.json');
