...
$path = SERVER_PATH_BLOGCMS."/data/blogs/$lsBlogID/images";
...
<div id="upload_area">
		...
		if ($handle = opendir($path)) {
			while (false !== ($file = readdir($handle))) {
				...
				if($ext == 'JPG' || $ext == 'PNG' || $ext == 'GIF' || $ext == 'JPEG') {
					echo '<img src="'.CLIENT_ROOT_ABS.'/projects/blog_cms/data/blogs/'.$lsBlogID.'/images/'.$file.'" height="100" width="" class="selectableimage" />';
				}
			}
			closedir($handle);
		}
	...
	<script type="text/javascript">
		...
		$("#frm_chooseNewImage").submit(function() {
			...
			// Make the ajax call
			ajax_PostFile("<?=CLIENT_ROOT_ABS?>/projects/blog_cms/ajax/submit_image_upload?blogid=<?=$lsBlogID?>&replace=1", formData, "showNew");
