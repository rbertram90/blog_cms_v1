...
{if isset($post)}
    ...
    {$formAction = "{$clientroot_blogcms}/posts/{$blog['id']}/edit/{$post['id']}/submit"}
...
{else}
    ...
    {$formAction = "{$clientroot_blogcms}/posts/{$blog['id']}/new/submit"}
    {$fieldTitle = ''}
    {$fieldContent = ''}
    {$fieldTags = ''}
    {$submitLabel = 'Create'}
...
<form action="{$formAction}" method="post" name="frm_createpost" id="frm_createpost">
    ...
    <script>
        ...
        function addNewField() {ldelim}
            $("#galleryNewImages").append('<div id="galleryimages' + fieldCount + 'holder"><div style="" class="galleryimage" id="galleryimages' + fieldCount + '"></div><input type="hidden" name="galleryimages[' + fieldCount + ']" value="" id="galleryimages' + fieldCount + 'path" class="galleryimagepath" /><button type="button" onclick="rbrtf_showWindow(\'{$clientroot_blogcms}/ajax/add_image?blogid={$blog.id}&elemid=galleryimages' + fieldCount + '&format=gallery\')" title="Insert Image">(*) Select Image</button><button type="button" onclick="removeImage(\'galleryimages' + fieldCount + 'holder\');">(-) Remove</button></div>');
            fieldCount++;
        {rdelim}
    ...
    {if $mode == 'edit'}
        ...
        {if strlen($galleryimage) > 0}
          <div id="galleryimages{$i}holder">
            ...
            <button type="button" onclick="rbrtf_showWindow('{$clientroot_blogcms}/ajax/add_image?blogid={$blog.id}&elemid=galleryimages{$i}&format=gallery')" title="Insert Image">(*) Change</button>
    ...
    <button type="button" onclick="rbrtf_showWindow('{$clientroot_blogcms}/ajax/add_image?blogid={$blog.id}')" title="Insert Image"><img src="{$clientroot}/resources/icons/document_image_add_32.png" style="width:15px; height:15px;" /></button>
...
{ldelim}
    ...
    {ldelim}
        ...
        {ldelim}
            ...
            jQuery.post("{$clientroot_blogcms}/ajax/autosave",
            {ldelim}
    ...
    {ldelim}
        ...
        var blog_id = {$blog.id};
        var url = "{$clientroot_blogcms}/ajax/check_title?blog_id=" + blog_id + "&post_title=" + post_title;
        ...
        if($(".rbrtf-rtfinput").is(":visible")) {ldelim}
            ...
            jQuery.get("{$clientroot}/core/ajax/ajax_viewWikiMarkup.php", {ldelim}content:htmlcontent{rdelim},  function(data) {ldelim}
