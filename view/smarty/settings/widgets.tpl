				...
				{foreach from=$sectionwidgets item=widget}
					<div class="draggablewidget" id="{$widget.type}-{$widget.id}">{$widget.type}
						<div style="float:right;">
							<a onclick="removeWidget('{$widget.type}-{$widget.id}'); return false;">Remove</a> | <a onclick="showWidgetSettings('{$widget.type}-{$widget.id}'); return false;">Options</a>
						</div>
...
<link href="{$clientroot}/resources/css/dragula.min.css" rel="stylesheet" type="text/css" />
...
<script>
	...
	**/
    function showWidgets(elem, location) {
		var newWindow = new rbwindow({
			url: '{$clientroot_blogcms}/ajax/widget_list?element=' + elem + '&location=' + location
		});
		newWindow.show();
	}
	...
	**/
	function showWidgetSettings(elemid) {
		...
		ajax_PostRequest('{$clientroot_blogcms}/ajax/widget_settings', config, function(data) {			
			var newWindow = new rbwindow({
				'htmlContent': data
			});
			newWindow.show();
	...
	**/
	var saveWidgetSettings = function(type, form) {
		ajax_PostRequest("{$clientroot_blogcms}/config/{$blog.id}/widgets/" + type + "/submit", encapsulateForm(form), function(data) {
			// Save Complete
