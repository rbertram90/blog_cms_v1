...
<form action="{$clientroot_blogcms}/config/{$blog.id}/stylesheet/submit" method="POST">
	...
	<p class="info"><i style="font-weight:normal;">Please beware this feature is targeted at
        ...
        <a href="{$clientroot_blogcms}/config/{$blog.id}/blogdesigner">blog designer</a>!</i>
    </p></label>
    
	<textarea name="fld_css" id="fld_css" style="height:500px; font-family:monospace;">{strip}
		{file_get_contents("{$serverroot_blogcms}/data/blogs/{$blog['id']}/default.css")}
	{/strip}</textarea>
