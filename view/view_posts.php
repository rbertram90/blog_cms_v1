...
require_once SERVER_PATH_BLOGCMS.'/app/view/view_post_summary.php';
...
function viewSinglePost($arrayPost, $arrayPostSettings, $prevPost="", $nextPost="") {
    ...
    $homefolder = CLIENT_ROOT_BLOGCMS;
?>
    ...
    <div class="post-footer">
        ...
        <?php if(isset($_SESSION['userid']) && $arrayPost['author_id'] == $_SESSION['userid']): ?>
            <a href="<?=$homefolder?>/posts/<?=$arrayPost['blog_id']?>/edit/<?=$arrayPost['id']?>">Edit</a> | 
            <a href="<?=$homefolder?>/posts/<?=$arrayPost['blog_id']?>/delete/<?=$arrayPost['id']?>" onclick='return confirm(\"Are you sure?\");'>Delete</a>
        <?php endif; ?>
...
function showTags($blogid, $tags) {
    ...
    foreach($arrayTags as $tag) {
        ...
        $output.= '<a href="'.CLIENT_ROOT_BLOGCMS.'/blogs/'.$blogid.'/tags/'.$tag.'" class="tag">'.$caption.'</a>';
    }
...
function viewMultiplePosts($arrayPosts, $blogid, $arrayPostSettings, $pTotalPostsOnBlog, $pCurrentPage) {
    ...
    {
        $blogdir = $homefolder.'/blogs/'.$blogid;
    }    
    ...
    <div class="post">
        ...
        <div class="post-footer">
            ...
            <p class='post-actions'>
                <a href="<?=$homefolder?>/posts/<?=$lobjPost['blog_id']?>/edit/<?=$lobjPost['id']?>">Edit</a> | 
                <a href="<?=$homefolder?>/posts/<?=$lobjPost['blog_id']?>/delete/<?=$lobjPost['id']?>" onclick='return confirm(\"Are you sure?\");'>Delete</a>
            </p>
            <?php endif; ?>
            ...
            <?php
                ...
                $encodedUrl   = rawurlencode(CLIENT_ROOT_ABS.$blogdir.'/posts/'.$lobjPost['link']);
                $unencodedUrl = CLIENT_ROOT_ABS.$blogdir.'/posts/'.$lobjPost['link'];
            ?>
            <style>.social-icons img { width:30px; height:30px; }</style>
            <div class="social-icons">
                <a href="https://www.facebook.com/sharer/sharer.php?u=<?=$encodedUrl?>" onclick="window.open(this.href, 'height=600,width=400'); return false;"><img src="<?=CLIENT_ROOT?>/resources/icons/social/facebook256.png" /></a>
                <a href="https://twitter.com/intent/tweet?url=<?=$encodedUrl?>&text=<?=$encodedTitle?>" target="_blank"><img src="<?=CLIENT_ROOT?>/resources/icons/social/twitter256.png" /></a>
                <a href="https://plus.google.com/share?url=<?=$unencodedUrl?>" target="_blank"><img src="<?=CLIENT_ROOT?>/resources/icons/social/googleplus256.png" /></a>
                <a href="mailto:?subject=<?=$encodedTitle?>&amp;body=<?=$encodedUrl?>"><img src="<?=CLIENT_ROOT?>/resources/icons/social/email256.png" /></a>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
?>
	...
	<form action="<?=CLIENT_ROOT_BLOGCMS?>/blogs/<?=$arrayBlog['id']?>/posts/<?=$arrayPost['link']?>/addcomment" method="POST">
